        ...
        switch ($this->File):
            ...
            case 'artigo':
                ...
                if (!$ReadSeo->getResult()):
                    ...
                    $this->Data = [$post_title . ' - ' . SITENAME, $post_content, BASE . "/artigo/{$post_name}", BASE . "/uploads/{$post_cover}"];
            ...
            //SEO:: CATEGORIA
            case 'categoria':
                $ReadSeo->ExeRead("categories", "WHERE category_name = :link", "link={$this->Link}");
                if (!$ReadSeo->getResult()):
                    $this->seoData = null;
                    $this->seoTags = null;
                else:
                    ...
                    $this->seoData = $ReadSeo->getResult()[0];
                    $this->Data = [$category_title . ' - ' . SITENAME, $category_content, BASE . "/categoria/{$category_name}", INCLUDE_PATH . '/images/site.png'];

                    //category:: conta views da categoria
                    $ArrUpdate = ['category_views' => $category_views + 1];
                    $Update = new Update();
                    $Update->ExeUpdate("categories", $ArrUpdate, "WHERE category_id = :catid", "catid={$category_id}");
                endif;
                break;
            ...
            case 'pesquisa':
                ...
                if (!$ReadSeo->getResult()):
                    ...
                    $this->Data = ["Pesquisa por: {$this->Link}" . ' - ' . SITENAME, "Sua pesquisa por {$this->Link} retornou {$this->seoData['count']} resultados!", BASE . "/pesquisa/{$this->Link}", INCLUDE_PATH . '/images/site.png'];
                endif;
                break;
            ...
            case 'empresas':
                ...
                $this->Data = ["Empresas {$this->Link}" . SITENAME, "Confira o guia completo de sua cidade, e encontra empresas {$this->Link}.", BASE . '/empresas/' . $this->Link, INCLUDE_PATH . '/images/site.png'];
                break;
            ...
            case 'cadastra-empresa':
                $this->Data = ["Cadastre sua Empresa - " . SITENAME, "Página modelo para cadastro de empresas via Front-End do curso Work Series - PHP Orientado a Objetos!", BASE . '/cadastra-empresa/' . $this->Link, INCLUDE_PATH . '/images/site.png'];
                break;

            //SEO:: INDEX
            case 'index':
                $this->Data = [SITENAME . ' - Sua revista online!', SITEDESC, BASE, INCLUDE_PATH . '/images/site.jpg'];
                break;

            //SEO:: 404
            default :
                $this->Data = [SITENAME . ' - 404 Oppsss, Nada encontrado!', SITEDESC, BASE . '/404', INCLUDE_PATH . '/images/site.jpg'];
