...
class AdminPost {
    ...
    public function ExeUpdate($PostId, array $Data) {
        ...
        if (in_array('', $this->Data)):
            $this->Error = ["Para atualizar esta postagem, preencha todos os campos!", WS_ALERT];
            $this->Result = false;
        else:
            $this->setData();
            $this->setName();
    ...
    public function ExeDelete($PostId) {
        ...
        if (!$ReadPost->getResult()):
            $this->Error = ["A postagem que você tentou deletar não existe no sistema!", WS_ERROR];
            $this->Result = false;
        else:
            ...
            $this->Error = ["A postagem <b>{$PostDelete['post_title']}</b> foi removido com sucesso!", WS_ACCEPT];
            $this->Result = true;
    ...
    private function Create() {
        ...
        if ($cadastra->getResult()):
            $this->Error = ["A postagem {$this->Data['post_title']} foi cadastrado com sucesso!", WS_ACCEPT];
    ...
    private function Update() {
        ...
        $Update->ExeUpdate(self::Entity, $this->Data, "WHERE post_id = :id", "id={$this->Post}");
        if ($Update->getResult()):
            $this->Error = ["A postagem <b>{$this->Data['post_title']}</b> foi atualizado com sucesso!", WS_ACCEPT];
            $this->Result = true;
        endif;
    }
