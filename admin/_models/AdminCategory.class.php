    ...
    public function ExeDelete($CategoryId) {
        ...
        if (!$read->getResult()):
            ...
            if (!$category_parent && !$this->checkCats()):
                $this->Result = false;
                $this->Error = ["A <b>categoria {$category_title}</b> possui sub-categorias. Para deletar, antes altere ou remova as sub-categorias!", WS_ALERT];
            elseif ($category_parent && !$this->checkPosts()):
                $this->Result = false;
                $this->Error = ["A <b>categoria {$category_title}</b> possui artigos cadastrados. Para deletar, antes altere ou remova todos as postegens desta categoria!", WS_ALERT];
            else:
                $delete = new Delete;
                $delete->ExeDelete(self::Entity, "WHERE category_id = :deletaid", "deletaid={$this->CatId}");

                $tipo = ( empty($category_parent) ? 'Categoria' : 'Sub-Categoria' );
                $this->Result = true;
                $this->Error = ["A {$tipo} <b>{$category_title}</b> foi removida do sistema!", WS_ACCEPT];
            endif;
        endif;
    }
